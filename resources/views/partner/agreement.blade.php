...
<div class="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl">
    ...
    <div>
        <div class="grid grid-cols-2 gap-4 items-center">
            ...
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ ucwords($partner->user->name) }}</p>

            <label class="text-sm font-medium text-gray-700">Email Address:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->user->email }}</p>

            <label class="text-sm font-medium text-gray-700">Partner Name:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ ucwords($partner->partner_name) }}</p>

            <label class="text-sm font-medium text-gray-700">ABN Number:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->abn_number }}</p>

            <label class="text-sm font-medium text-gray-700">Address:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->operation_location }}</p>
        </div>
    </div>
<br>
...
<br>
    <div>
        <div class="grid grid-cols-2 gap-4 items-center">
            ...
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->billing_commencement_period ?? 'N/A' }}</p>

{{--            <label class="text-sm font-medium text-gray-700">Currency:</label>--}}
{{--            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->currency }}</p>--}}

            <label class="text-sm font-medium text-gray-700">Establishment Fee:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->establishment_fee }}</p>

            <label class="text-sm font-medium text-gray-700">Monthly Subscription Fee:</label>
            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->monthly_subscription_fee }}</p>

{{--            <label class="text-sm font-medium text-gray-700">Admin Fee for Freelance Buyers:</label>--}}
{{--            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->admin_fee }}</p>--}}

{{--            <label class="text-sm font-medium text-gray-700">Commission Charge for Freelance Buyers:</label>--}}
{{--            <p class="border-2 rounded-full px-4 py-2 text-sm" style="border-color: #5271FF;">{{ $partner->comm_charge }}</p>--}}
        </div>
    </div>
    ...
    <div class="mt-6">
        ...
        <ul class="list-disc pl-5 mt-2 text-left">
            <li>
                ...
                <a href="{{ asset('storage/' . $admin->master_agreement_for_partners) }}" target="_blank" class=" hover:underline" style="color: #5271FF;">View Agreement</a>
            </li>
            <li>
                Terms of Service with WConsole:
                <a href="{{ asset('storage/' . $admin->terms_conditions_wc_partners) }}" target="_blank" class=" hover:underline" style="color: #5271FF;">Terms of Service</a>
            </li>
            <li>
                Service Schedule with WConsole:
                <a href="{{ asset('storage/agreements/' . basename($partner->your_agreement)) }}" target="_blank" class=" hover:underline" style="color: #5271FF;">Service Schedule</a>
            </li>
        </ul>
    </div>
    ...
    <div>
        <form id="agreement-form" action="{{ route('partneragreement.submit', [$user->id]) }}" method="POST" class="mt-4">
            @csrf
