...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <!-- Buttons to toggle between Partner and Consoler Invoices -->
                <div class="mb-3">
                    <button id="consolerButton" class="btn btn-primary">Consoler Invoices</button>
                    <button id="partnerButton" class="btn button border-2">Partner Invoices</button>
                </div>

                <!-- Consoler Invoices Table -->
                <div class="table-responsive" id="consolerTable">
                    <table class="table table-borderless" width="100%" cellspacing="0">
                        ...
                        <tbody>
                        @foreach($invoices->whereNull('partner_id') as $invoice)
                            <tr>
                ...
                <div class="table-responsive" id="partnerTable" style="display: none;">
                    <table class="table table-borderless" width="100%" cellspacing="0">
                        <thead style="pointer-events: none; user-select: none; color:#5271FF">
                        <tr>
                            <th>Date Created</th>
                        ...
                        <tbody>
                        @foreach($invoices->whereNotNull('partner_id') as $invoice)
                            <tr>
                                <td>{{ \Carbon\Carbon::parse($invoice->date_created)->format('d M, Y') }}</td>
                                <td>{{ $invoice->invoice_number }}</td>
                                <td>{{ ucwords(strtolower($invoice->consoler_name)) }}</td>
                                ...
                                <td>
                                    <a href="{{ route('partnerinvoices.pdf', $invoice->id) }}" class="btn border-2" style="color:#5271FF; border-color: #5271FF">Download</a>
                                    <button type="button" class="btn border-2 view-invoice-partner-btn"  style="color:#5271FF; border-color: #5271FF" data-invoice-id="{{ $invoice->id }}">View</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
@section('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // View Invoice Button Logic
            document.querySelectorAll(".view-invoice-partner-btn").forEach(button => {
                button.addEventListener("click", function () {
                    const invoiceId = this.getAttribute("data-invoice-id");
                    fetch(`/invoices/${invoiceId}/partner`)
                        .then(response => response.text())
                        .then(html => {
                            document.getElementById("invoiceContent").innerHTML = html;
                            modal.show();
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            const consolerButton = document.getElementById("consolerButton");
            ...
            consolerButton.addEventListener("click", () => {
                consolerTable.style.display = "block";
                partnerTable.style.display = "none";
                consolerButton.classList.add("btn-primary");
                consolerButton.classList.remove("button");
                partnerButton.classList.remove("btn-primary");
            ...
            partnerButton.addEventListener("click", () => {
                ...
                partnerButton.classList.add("btn-primary");
                partnerButton.classList.remove("button");
                consolerButton.classList.remove("btn-primary");
                consolerButton.classList.add("button");
            });
        });
    </script>
@endsection
